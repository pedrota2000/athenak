#!/bin/bash
#SBATCH --job-name=turb_1024_viscous_4gpu
#SBATCH -p gpuxl
#SBATCH -C h100
...
#SBATCH --mem=0                 
#SBATCH --output=turb_1024_viscous_%j.out
#SBATCH --error=turb_1024_viscous_%j.err
...
run_dir=/mnt/ceph/users/ptarancon/runs/turb_1024_viscous_compressible_run_v2_$(date +%Y%m%d_%H%M%S)
mkdir -p $run_dir
cd $run_dir

# Copy input file
cp $athenak/inputs/compressible_simulations/3d_turb_big_box_viscous.athinput ./input.athinput

# Configuration for cleanup
KEEP_LAST_N_BIN=10      # Keep last 25 bin files
