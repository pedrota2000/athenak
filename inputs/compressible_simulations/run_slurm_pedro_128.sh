...
run_dir=/mnt/ceph/users/ptarancon/runs/turb_128_compressible_run_v2_$(date +%Y%m%d_%H%M%S)
mkdir -p $run_dir
cd $run_dir

# Copy input file
cp $athenak/inputs/compressible_simulations/3d_turb_small.athinput ./input.athinput

# Configuration for cleanup
KEEP_LAST_N_BIN=100      # Keep last 100 bin files
...
if [ $file_count -gt $KEEP_LAST_N_BIN ]; then
    deleted_count=$((file_count - KEEP_LAST_N_BIN))
    ls -1v ${run_dir}/vtk/Turb.*.vtk | head -n ${deleted_count} | xargs rm -f
fi
