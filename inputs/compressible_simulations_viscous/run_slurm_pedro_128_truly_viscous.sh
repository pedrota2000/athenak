#!/bin/bash
#SBATCH --job-name=turb_128_truly_visc_4gpu
#SBATCH -p gpuxl
#SBATCH -C h100
...
#SBATCH --mem=0
#SBATCH --output=turb_128_truly_viscous_%j.out
#SBATCH --error=turb_128_truly_viscous_%j.err
...
build=$athenak/build
input=$athenak/inputs/compressible_simulations_viscous/3d_turb_128_truly_viscous.athinput
...
if [ -n "${RESUME_DIR:-}" ]; then
    ...
    run_dir=/mnt/ceph/users/ptarancon/runs/turb_128_compressible_run_truly_viscous_$(date +%Y%m%d_%H%M%S)
...
KEEP_LAST_N_BIN=100   # same snapshot count as the matching inviscid run
...
echo "Starting 128^3 turbulence simulation (RESOLVED PHYSICAL VISCOSITY) at $(date)"
