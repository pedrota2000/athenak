#!/bin/bash
#SBATCH --job-name=turb_512_truly_visc_4gpu
#SBATCH -p gpuxl
#SBATCH -C h100
...
#SBATCH --mem=0
#SBATCH --output=turb_512_truly_viscous_%j.out
#SBATCH --error=turb_512_truly_viscous_%j.err
...
build=$athenak/build
input=$athenak/inputs/compressible_simulations_viscous/3d_turb_512_truly_viscous.athinput
...
if [ -n "${RESUME_DIR:-}" ]; then
    ...
    run_dir=/mnt/ceph/users/ptarancon/runs/turb_512_viscous_compressible_run_truly_viscous_$(date +%Y%m%d_%H%M%S)
...
KEEP_LAST_N_BIN=25   # same snapshot count as the matching inviscid run
...
echo "Starting 512^3 turbulence simulation (RESOLVED PHYSICAL VISCOSITY) at $(date)"
